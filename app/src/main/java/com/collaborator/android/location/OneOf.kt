package com.moviebuff.domain.entities
...
sealed class OneOf<out S, out F : Failure> {
    ...
    data class Failure<F : com.moviebuff.domain.entities.Failure>(val b: F) : OneOf<Nothing, F>()

    val isFailure get() = this is Failure<F>
    val isSuccess get() = this is Success<S>

    fun <S> success(a: S, dataState: Success.DataState) = Success(a, dataState)
    fun <F : com.moviebuff.domain.entities.Failure> failure(b: F) = Failure(b)
